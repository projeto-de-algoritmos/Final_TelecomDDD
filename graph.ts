...
function tamanho_array(Array: any[]) {
    let size: number = 0;
    while (Array[size] != undefined) {
        size++;
    }
    return size;
}

function calculate_distance(pos1: citys, pos2: citys) {
    return Math.sqrt((pos1.longitude - pos2.longitude) ** 2 + (pos1.latitude - pos2.latitude) ** 2);
}

function Gerar_grafo(cidades: citys[] , range_cabos:number) {
    const size = tamanho_array(cidades);
...
function Dijask(initial_index:number, final_index:number, graph:number[][]) {
    var priorityquery = [];
    var visited = [];
    const grafosize = tamanho_array(graph);
...
export { Gerar_grafo, Dijask, tamanho_array };
// distancias em NM
